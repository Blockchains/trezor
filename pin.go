...
//
// This library implements a full-screen terminal-based UI for PIN entry. It is used in
...
func GetPIN(prompt string) (string, error) {
	...
	printStr := func(x, y int, s string) {
		i := 0 // Rune index. (Not using the index from `range` because it's a byte index. See [https://blog.golang.org/strings#TOC_6.])
		for _, r := range s {
